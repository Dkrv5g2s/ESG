...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "After train-only validation, retrain the final submission model on "
    ...
    parser.add_argument(
        ...
        dest="merge_train_val_for_submission",
        action="store_false",
        help="Keep the validation-best train-only model for the submission CSV.",
    )
    parser.add_argument(
        "--final-epochs",
        type=int,
        default=DEFAULT_FINAL_EPOCHS,
        help=(
            "Epochs for the final train+validation run. Defaults to the fixed "
            "8-epoch setting from train_val_merge.py."
        ),
    )
